...
const DUMMY_WORKSHOPS = [
  { id: "WS-001", title: "Morning Vinyasa Flow", category: "Yoga", instructor: "Dr. Rekha Menon", date: "2026-06-15", time: "07:00 AM", duration: 90, capacity: 20, enrolled: 18, price: 1500, status: "upcoming", description: "A dynamic yoga session focused on breath-synchronized movement to energize the body and calm the mind." },
  { id: "WS-002", title: "Stress Relief Meditation", category: "Meditation", instructor: "Dr. Arjun Nair", date: "2026-06-18", time: "06:30 PM", duration: 60, capacity: 25, enrolled: 25, price: 800, status: "full", description: "Guided meditation techniques for deep relaxation and stress reduction, suitable for all levels." },
  { id: "WS-003", title: "Ayurvedic Nutrition Workshop", category: "Nutrition", instructor: "Dr. Priya Krishnan", date: "2026-06-20", time: "10:00 AM", duration: 120, capacity: 15, enrolled: 11, price: 2500, status: "upcoming", description: "Learn the principles of Ayurvedic nutrition, seasonal eating, and customized diet planning." },
  { id: "WS-004", title: "Sound Healing & Chakra", category: "Holistic", instructor: "Guru Anandamaya", date: "2026-06-22", time: "05:00 PM", duration: 90, capacity: 12, enrolled: 7, price: 3000, status: "upcoming", description: "Experience the healing power of Tibetan singing bowls and chakra balancing for deep inner peace." },
  { id: "WS-005", title: "Pranayama Intensive", category: "Yoga", instructor: "Dr. Sanjay Bhat", date: "2026-06-14", time: "07:00 AM", duration: 60, capacity: 20, enrolled: 20, price: 1200, status: "ongoing", description: "An intensive breathwork program covering all major pranayama techniques for vitality and longevity." },
  { id: "WS-006", title: "Gut Health & Digestion", category: "Nutrition", instructor: "Dr. Priya Krishnan", date: "2026-06-10", time: "11:00 AM", duration: 90, capacity: 20, enrolled: 20, price: 2000, status: "completed", description: "Evidence-based workshop on gut microbiome, probiotics, and digestive wellness through Ayurveda." },
  { id: "WS-007", title: "Mindfulness & Sleep Science", category: "Meditation", instructor: "Dr. Arjun Nair", date: "2026-06-25", time: "08:00 PM", duration: 60, capacity: 30, enrolled: 14, price: 600, status: "upcoming", description: "Combine mindfulness practices with sleep hygiene science for restorative and rejuvenating sleep." },
  ...
  { id: "WS-009", title: "Flexibility & Mobility", category: "Yoga", instructor: "Dr. Rekha Menon", date: "2026-07-02", time: "08:00 AM", duration: 75, capacity: 18, enrolled: 9, price: 1000, status: "upcoming", description: "A therapeutic yoga session targeting joint mobility, flexibility and injury prevention." },
  { id: "WS-010", title: "Detox & Cleanse Program", category: "Ayurveda", instructor: "Dr. Kavitha Rao", date: "2026-06-08", time: "09:00 AM", duration: 180, capacity: 8, enrolled: 8, price: 5000, status: "completed", description: "Three-hour deep dive into Panchakarma principles for body purification and cellular renewal." },
];
...
function WorkshopCard({ w, onClick }) {
  ...
  return (
    <div className="ws-card" onClick={() => onClick(w)}>
      <div className="ws-card-banner" style={{ background: `linear-gradient(135deg, ${cat.color}22, ${cat.color}44)` }}>
        <div className="ws-card-category-badge" style={{ background: cat.color, color: "white" }}>{w.category}</div>
        <div className="ws-card-status-badge" style={{ background: st.bg, color: st.color, border: `1px solid ${st.color}44` }}>{st.label}</div>
        <div className="ws-card-category-icon" style={{ color: cat.color }}>
...
export default function Workshops() {
  ...
  const handleToggleTicket = async (ticketId, currentStatus) => {
  ...
  const FormField = ({ label, children }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      <label style={{ fontSize: 12, fontWeight: 700, color: "#4a5568", textTransform: "uppercase", letterSpacing: "0.4px" }}>{label}</label>
      {children}
    </div>
  );
          ...
          <div className="ws-modal" style={{ width: 580 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", borderBottom: "1px solid #f1f3f7", padding: "0 28px" }}>
              {["info", "attendees"].map(t => (
                ...
                >
                  {t === "info" ? "📋 Info" : `🎟️ Attendees (${attendees.length})`}
                </button>
              ))}
            </div>
                            ...
                            <td style={{ padding: "10px 12px" }}>
                              <div style={{ display: "flex", gap: 6 }}>
                                {a.ticket_status !== "cancelled" && (
                                  <button
                                    onClick={() => handleToggleTicket(a.ticket_id, a.ticket_status)}
                                    style={{
            ...
            <div className="ws-modal-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <FormField label="Program Title *">
                <input name="title" value={addForm.title} onChange={handleAddFormChange}
                  style={inputStyle} placeholder="e.g. Sunset Yoga Flow" />
              </FormField>

              {/* Category + Status */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <FormField label="Category *">
                  <select name="category" value={addForm.category} onChange={handleAddFormChange} style={inputStyle}>
                    {Object.keys(CATEGORY_COLORS).map(c => <option key={c}>{c}</option>)}
                  </select>
                </FormField>
                <FormField label="Status">
                  <select name="status" value={addForm.status} onChange={handleAddFormChange} style={inputStyle}>
              ...
              <FormField label="Instructor Name *">
                <input name="instructor" value={addForm.instructor} onChange={handleAddFormChange}
                  style={inputStyle} placeholder="e.g. Dr. Priya Krishnan" />
              </FormField>

              {/* Date + Time */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <FormField label="Date *">
                  <input type="date" name="date" value={addForm.date} onChange={handleAddFormChange} style={inputStyle} />
                </FormField>
                <FormField label="Start Time">
                  <input type="time" name="time" value={addForm.time} onChange={handleAddFormChange} style={inputStyle} />
                </FormField>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
                <FormField label="Duration (mins)">
                  <input type="number" name="duration" value={addForm.duration} onChange={handleAddFormChange} style={inputStyle} min={15} step={15} />
                </FormField>
                <FormField label="Capacity (seats)">
                  <input type="number" name="capacity" value={addForm.capacity} onChange={handleAddFormChange} style={inputStyle} min={1} />
                </FormField>
                <FormField label="Price (₹) *">
                  <input type="number" name="price" value={addForm.price} onChange={handleAddFormChange} style={inputStyle} placeholder="1500" min={0} />
                </FormField>
              </div>
              ...
              <div style={{ display: "flex", gap: 12, marginTop: 4 }}>
                ...
                <button
                  className="ws-modal-btn-primary"
                  style={{ flex: 2, background: "#CDA751" }}
