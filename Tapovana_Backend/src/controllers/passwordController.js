...
const requestPasswordResetOtp = async (req, res) => {
    ...
    try {
        const memberResult = await query(
            "SELECT tm.id, tm.email, tm.first_name, tm.status FROM team_members tm WHERE LOWER(tm.email) = LOWER($1)",
            [email]
        );
        ...
        await query(
            "UPDATE otp_verification SET used = TRUE WHERE member_id = $1 AND otp_type = 'password_reset' AND used = FALSE",
            [member.id]
        );
        ...
        await query(
            "INSERT INTO otp_verification (member_id, otp_code, otp_type, expires_at) VALUES ($1, $2, 'password_reset', $3)",
...
const verifyPasswordResetOtpAndSetPassword = async (req, res) => {
    ...
    try {
        ...
        const memberResult = await client.query(
            "SELECT tm.id, tm.email, tm.first_name, lc.id AS cred_id FROM team_members tm JOIN login_credentials lc ON lc.member_id = tm.id WHERE LOWER(tm.email) = LOWER($1)",
            [email]
        );
        ...
        const otpResult = await client.query(
            "SELECT id, otp_code, attempts FROM otp_verification WHERE member_id = $1 AND otp_type = 'password_reset' AND used = FALSE AND expires_at > NOW() ORDER BY created_at DESC LIMIT 1",
            [member.id]
        );
        ...
        await client.query("UPDATE otp_verification SET used = TRUE WHERE id = $1", [otpRow.id]);
        await client.query(
            "UPDATE login_credentials SET password_hash = $1, temp_password_hash = NULL, must_change = FALSE, reset_token = NULL, reset_token_expiry = NULL, updated_at = NOW() WHERE id = $2",
...
const changePassword = async (req, res) => {
    ...
    try {
        ...
        await query(
            "UPDATE login_credentials SET password_hash = $1, temp_password_hash = NULL, must_change = FALSE, updated_at = NOW() WHERE id = $2",
            [newHash, cred.id]
        );
...
// ─── FIXED: Uses 'password_reset' instead of 'force_change' ───────────────
const requestForceChangeOtp = async (req, res) => {
    try {
        ...
        console.error("requestForceChangeOtp error:", err);
        return res.status(500).json({ success: false, message: "Server error." });
    }
};

// ─── FIXED: Uses 'password_reset' instead of 'force_change' ───────────────
const verifyForceChangeOtpAndSetPassword = async (req, res) => {
    ...
    try {
        ...
        await client.query(
            "UPDATE login_credentials SET password_hash = $1, temp_password_hash = NULL, must_change = FALSE, updated_at = NOW() WHERE member_id = $2",
            [newHash, memberId]
        );
        ...
        await client.query("ROLLBACK");
        console.error("verifyForceChangeOtpAndSetPassword error:", err);
        return res.status(500).json({ success: false, message: "Server error." });
    } finally {
        client.release();
    }
