...
const handleProfilePhotoSave = (source, url, base64) => {
    if (source === 'upload' && base64) {
        ...
        if (match) {
            return { url: match[0], source: 'upload' };
        }
...
const addTeamMember = async (req, res) => {
    const { first_name, last_name, email, phone, role,
        profile_photo_source, profile_photo_url, profile_photo_base64 } = req.body;
    ...
    try {
        ...
        const existing = await client.query('SELECT id FROM team_members WHERE LOWER(email) = LOWER($1)', [email]);
       ...
       RETURNING id`,
            [first_name, last_name, email.toLowerCase(), phone || null, roleId, req.user.id,
                photoResult.url, photoResult.source, photoResult.url]
        );
       ...
       VALUES ($1, $2, TRUE)`,
        ...
        sendWelcomeEmail({ to: email, firstName: first_name, tempPassword }).catch(console.error);

        return res.status(201).json({ success: true, message: 'Team member added. Invite email sent.', member_id: memberId });
    } catch (err) {
        await client.query('ROLLBACK');
...
const updateTeamMember = async (req, res) => {
    const { first_name, last_name, phone, role, status, avatar_url,
        profile_photo_source, profile_photo_url, profile_photo_base64,
        availability_status, allocation_details } = req.body;
    ...
    try {
        ...
        if (profile_photo_source) {
            const photoResult = handleProfilePhotoSave(profile_photo_source, profile_photo_url, profile_photo_base64);
            fields.push(`profile_photo_source = $${idx++}`); values.push(photoResult.source);
            fields.push(`profile_photo_url = $${idx++}`); values.push(photoResult.url);
            fields.push(`avatar_url = $${idx++}`); values.push(photoResult.url);
        }
        ...
        if (role) {
            ...
            if (!roleResult.rows.length) return res.status(400).json({ success: false, message: 'Invalid role.' });
        ...
        if (!result.rows.length) return res.status(404).json({ success: false, message: 'Member not found.' });
...
const deleteTeamMember = async (req, res) => {
    try {
        ...
        const result = await query(
            `UPDATE team_members SET status = 'inactive', updated_at = NOW() WHERE id = $1 RETURNING id`,
            [req.params.id]
        );
        if (!result.rows.length) return res.status(404).json({ success: false, message: 'Member not found.' });
...
const resendInvite = async (req, res) => {
    ...
    try {
       ...
       WHERE tm.id = $1 AND tm.status = 'pending'`,
        ...
        await client.query(
            `UPDATE login_credentials SET temp_password_hash = $1, must_change = TRUE, updated_at = NOW() WHERE id = $2`,
            [tempHash, cred_id]
        );
        await client.query('COMMIT');
...
const addTeamMemberFrontend = async (req, res) => {
    ...
    try {
        ...
        const memberResult = await client.query(
            `INSERT INTO team_members (first_name, last_name, email, phone, role_id, specialization,
              status, created_by, profile_photo_url, profile_photo_source, avatar_url)
       VALUES ($1, $2, $3, $4, $5, $6, 'pending', $7, $8, $9, $10)
       RETURNING id`,
            [first_name, last_name, email.toLowerCase(), phone || null, roleId,
       ...
       VALUES ($1, $2, TRUE)`,
        ...
        return res.status(201).json({ success: true, message: 'Team member added. Invite email sent.', user_id: memberId });
    } catch (err) {
        await client.query('ROLLBACK');
...
const updateTeamMemberFrontend = async (req, res) => {
    const { first_name, last_name, phone, role, status, specialization, avatar_url,
        profile_photo_source, profile_photo_url, profile_photo_base64,
        availability_status, allocation_details } = req.body;
...
const toggleStatusFrontend = async (req, res) => {
    ...
    try {
        const dbStatus = status === 'ACTIVE' ? 'active' : 'inactive';
        const result = await query(
            `UPDATE team_members SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING id`,
        ...
        const result = await query(
            `UPDATE team_members SET ${fields.join(', ')}, updated_at = NOW()
             ...
             RETURNING id, first_name, last_name, email, phone, role_id,
                       status, profile_photo_url, profile_photo_source`,
            values
        );
